...
function includesAt(lst, pattern, index) {
  ...
  for (let i = 0; i < pattern.length; i++)
    if (pattern[i] !== lst[index + i]) return false;
  return true;
}
...
export class Bitmap {
  ...
  constructor(size, data) {
    ...
    this.data =
      data || Array.from({ length: height }, () => fillArr(width, undefined));
    this.height = height;
    this.width = width;
  }
  point(p) {
...
function drawTemplate(ver, ecc, maskIdx, test = false) {
  ...
  // Timing patterns
  b = b
    .hLine({ x: 0, y: 6 }, Infinity, ({ x }, cur) =>
      cur === undefined ? x % 2 == 0 : cur,
    )
    .vLine({ x: 6, y: 0 }, Infinity, ({ y }, cur) =>
...
export function encodeQR(text, output = "raw", opts = {}) {
  ...
  validateECC(ecc);
  const encoding =
    opts.encoding !== undefined ? opts.encoding : detectType(text);
